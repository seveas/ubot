...
Ubot.fn.initialize = function(botname) {
    ...
    $('html').ajaxError(function(event, xhr, option, error) {
        ...
        msg = xhr.readyState ? xhr.responseText : "Ubot control server is not running";
        var dialog = $('<div id="dialog-confirm" title="System error">' +
            '<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>' + msg + '</p></div>');
        dialog.dialog({
            resizable: false,
            height: 170,
            width: 400,
    ...
    $(document).ready(function() {
        ...
        $("#robot").bind({
            'mouseenter': function(){
                if($("#robot").data("animating")) {
                    return;
                }
                if($("#robot").css("z-index") == 4) {
                    return;
                }
                $("#robot").data("animating", 1);
                $("#robot").animate(
                    {'bottom': '15'},
                    {
                        'duration': 500,
...
Ubot.fn.get_info = function() {
    ...
    $.ajax({
        ...
        success: function(info) {
            ...
            if(info.connected) {
                $('#connection_info').html(info.server + ':' + info.port + ' (' + escape_html(info.server_version) + ')');
            }
            else {
...
Ubot.fn.activate_channel = function(header) {
    ...
    if(header && header.attr('id')) {
        var name = header.attr('id').substr(8);
        bot.get_topic(name);
        bot.get_mode(name);
        bot.get_nicks(name);
...
Ubot.fn.channel_widgets = function(name) {
    ...
    var html = S('<p><b>Topic:</b> <span id="topic_%NAME%">&nbsp;</span></p>' +
        ...
        '<table width="100%">' +
          '<tr>' +
            '<td width="75%"><input type="text" id="input_say_%NAME%" class="input-hint" value="Message" /></td>' +
            '<td width="5%">&nbsp;</td>' +
            '<td width="20%"><button id="button_say_%NAME%"">Say</button></td>' +
          '</tr><tr>' +
...
Ubot.fn.channel_scripts = function(name) {
    ...
    $('#input_say_' + escape_selector(name)).bind({
        focus: function() {
            if($(this).hasClass('input-hint')) {
                $(this).removeClass('input-hint');
                this.value = '';
        ...
        blur: function() {
            if(this.value == '' || this.value == $('#say_' + escape_selector(name)).html()) {
                this.value = 'Message'
                $(this).addClass('input-hint');
            }
        }
    });
    $('#button_say_' + escape_selector(name)).click(function() {
        var message = $("#input_say_" + escape_selector(name)).attr('value');
        if(message != "Message") {
            bot.channel_say(name, message);
        }
    });
    $('#button_say_' + escape_selector(name)).button();
...
Ubot.fn.channel_say = function(name, message) {
    var bot = this;
    var is_action = /^\/me /i.test(message);
    if(is_action) {
        message = message.substr(4);
    ...
    $.ajax({
        type: 'POST',
        url: '../rest/' + this.botname + '/channel/' + escape(name) + '/' + (is_action ? 'do' : 'say') + '/',
        data: 'message=' + escape(message),
        dataType: 'json',
        success: function(result) {
            if(result.status == 'error') {
                bot.alert('Can\'t send message: ' + result.error);
                return;
            }
            var input = $("#input_say_" + escape_selector(name));
            input.attr('value', 'Message sent!');
            input.addClass('input-hint');
        }
    });
}
...
Ubot.fn.activate_helper = function(header) {
    ...
    bot.interval_
    if(header && header.attr('id')) {
        var name = header.attr('id').substr(7);
